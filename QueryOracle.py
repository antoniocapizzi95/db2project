...
class QueryOracle(object) :
    ...
    def chooseQuery(self,number):
        switcher = {
            ...
            2: "Select denomination, forme_juridique.name as forme_juridique, code_ape.name as code_ape from company,forme_juridique,code_ape where company.id_formjur = forme_juridique.id AND company.id_codeape = code_ape.id",
            3: "",
            4: "select denomination, addresse.name as adresse,code_postal,ville.name as ville from company, addresse,ville where company.id_addresse = addresse.id and addresse.id_ville = ville.id",
            5: "May",
            6: "June",
            7: "July"
        }

...
class QueryNeo4j(object) :
    def query1(self,n4j,queryNum):
        ...
        for i in range(31):
            ...
            print(result)
            l.append(str(result))
        self.writeCsv("Neo4j",str(queryNum),"10000",l)
    ...
    def chooseQuery(self,number):
        switcher = {
            ...
            6: "MATCH p=(c:Company)-[r:HAS_FORMEJURIDIQUE]->(fj:FormeJuridique),a=(c)-[:HAS_ADRESSE]->(add:Adresse),s=(add)-[:HAS_CODEPOSTAL]->(co:CodePostal),f=(co)-[:HAS_CITY]->(ci:Ville),g=(ci)-[:HAS_REGION]->(re:Region) WHERE ID(re)=10793 RETURN fj,count(c)",
